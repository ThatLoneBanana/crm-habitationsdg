...
export default function VueClientPage({ params: paramPromise }: { params: Promise<{ slug: string }> }) {
  ...
  useEffect(() => {
    const fetchProjet = async () => {
      try {
        const res = await fetch(`/api/projets-by-slug?slug=${params.slug}`);
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2">
              {extrasSignes.map((e: any) => (
                <div key={e.id} className="bg-green-50 border border-green-200 rounded-lg p-3">
                  <div className="flex justify-between items-start">
                    ...
                    <p className="font-semibold text-green-700">{formatCurrency(e.montant)}</p>
                  </div>
                </div>
              ))}
              <div className="bg-green-100 border border-green-300 rounded-lg p-3 flex justify-between items-center">
                ...
                <p className="font-bold text-green-900">{formatCurrency(totalExtrasSignes)}</p>
              </div>
            </div>
          </div>
        )}
          ...
          <div className="space-y-2">
            {projet.paiements.map((p: any) => (
              <div key={p.id} className={`border rounded-lg p-3 ${p.recu ? 'bg-green-50 border-green-200' : 'bg-yellow-50 border-yellow-200'}`}>
                ...
                <div className="flex justify-between items-center">
                  ...
                  <p className="font-semibold text-gray-900">{formatCurrency(p.montant)}</p>
                </div>
              </div>
            ))}
            <div className="bg-blue-100 border border-blue-300 rounded-lg p-3">
              <div className="flex justify-between items-center">
                ...
                <p className="font-bold text-blue-900">{formatCurrency(totalRecu)} / {formatCurrency(totalPlanifie)}</p>
              </div>
            </div>
          </div>
        </div>
