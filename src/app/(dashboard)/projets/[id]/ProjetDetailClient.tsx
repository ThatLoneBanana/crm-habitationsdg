...
// serveur, en parallèle — fin du waterfall). Aucun fetch au montage. users
...
export default function ProjetDetailClient({ projet, parametres }: ProjetDetailClientProps) {
  ...
  const margeCeduleJours = parametres?.margeCeduleJours ?? 5;
  const parametresEntreprise = parametres;

  // users : nécessaire seulement au dialog Modifier (selects vendeur/chargé) et
  // à la création de cédule (liste fournisseurs) → chargé à l'ouverture.
  const ensureUsers = async () => {
    if (usersLoaded) return;
    setUsersLoaded(true);
    try {
      const res = await fetch('/api/users');
      if (res.ok) { const d = await res.json(); setUtilisateurs(d.users || []); }
    } catch { /* silencieux */ }
  };

  // costing : nécessaire seulement à l'onglet Costing → chargé à son ouverture.
  const ensureCosting = async () => {
    if (costingLoaded || costingLoading) return;
    setCostingLoading(true);
    try {
      const res = await fetch(`/api/projets/costing?projetId=${projet.id}`);
      if (res.ok) { const d = await res.json(); setCosting(d.calculs); }
    } catch { /* silencieux */ }
  ...
  // projet est garanti par le Server Component (notFound() sinon) — garde de sûreté.
      ...
      <div>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 20, flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            <Button variant="outline" className="gap-2" onClick={() => { setModifierOpen(true); ensureUsers(); }}>
          ...
          <TabsContent value="cedule" className="m-0">
            {projet.taches.length === 0 ? (
              <div style={{
                ...
                <button
                  onClick={() => { setCedulaModalOpen(true); ensureUsers(); }}
                  style={{
                    padding: '10px 24px',
                    background: '#1D9E75',
                    color: 'white',
